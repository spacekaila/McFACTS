...
def bin_tgw(mass_1,mass_2,location_1,location_2,norm_t_gw):
    ...
    bin_mass = mass_1 + mass_2
    bin_reduced_mass = (mass_1*mass_2)/bin_mass
    bin_separation = np.abs(location_1-location_2)

    scaled_location = 1.e3
    scaled_bin_mass = 10.0
    ...
    #     bin_t_gw=norm_t_gw*((bin_separation)^(4.0))*((m_smbh/1.e8)^(4.0))*((bin_mass/10.0)^(-2.0))*((bin_reduced_mass/2.5)^(-1.0))
