...
def feedback_hankla_stars(prograde_stars_locations, disk_surf_model, frac_star_Eddington_ratio, alpha):
    """_summary_
    ...
    if isinstance(disk_surf_model, float):
        ...
        disk_surface_density = disk_surf_model(prograde_stars_locations)
    ...
    Ratio_feedback_migration_torque = 0.* 0.07 *(1/kappa)* ((alpha)**(-1.5))*frac_star_Eddington_ratio*np.sqrt(prograde_stars_locations)/disk_surface_density

    #print((1/kappa),((alpha)**(-1.5)),frac_star_Eddington_ratio)
    #print("Ratio", Ratio_feedback_migration_torque) 
    #print("Stars locations", prograde_stars_locations) 
