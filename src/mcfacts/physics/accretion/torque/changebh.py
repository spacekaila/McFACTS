...
def change_spin_magnitudes(disk_bh_pro_spins, disk_bh_eddington_ratio, disk_bh_torque_condition, timestep_duration_yr, disk_bh_pro_orbs_ecc, disk_bh_pro_orbs_ecc_crit):
    ...
    bh_new_spins = disk_bh_pro_spins
    ...
    bh_new_spins[indices_bh_spin_up] = disk_bh_pro_spins[indices_bh_spin_up] + spin_iteration
    #Spin down BH with orb ecc > e_crit
    bh_new_spins[indices_bh_spin_down] = disk_bh_pro_spins[indices_bh_spin_down] - spin_iteration
...
def change_spin_angles(disk_bh_pro_spin_angles, disk_bh_eddington_ratio, disk_bh_torque_condition, disk_bh_spin_minimum_resolution, timestep_duration_yr, disk_bh_pro_orbs_ecc, disk_bh_pro_orbs_ecc_crit):
    """Update the spin magnitude of the embedded black holes based on their accreted mass
        in this timestep.
    ...
    bh_new_spin_angles = disk_bh_pro_spin_angles
    #Singleton BH with orb ecc > e_crit will spin down b/c accrete retrograde
    prograde_bh_spin_down = np.ma.masked_where(disk_bh_pro_orbs_ecc <= disk_bh_pro_orbs_ecc_crit, disk_bh_pro_orbs_ecc)
    #Singleton BH with orb ecc < e_crit will spin up b/c accrete prograde
    prograde_bh_spin_up = np.ma.masked_where(disk_bh_pro_orbs_ecc >disk_bh_pro_orbs_ecc_crit, disk_bh_pro_orbs_ecc)
    ...
    bh_new_spin_angles[indices_bh_spin_up] = disk_bh_pro_spin_angles[indices_bh_spin_up] - spin_torque_iteration
    #Spin down BH with orb ecc > e_crit are torqued toward anti-alignment with disk, incr mag of spin angle.
    bh_new_spin_angles[indices_bh_spin_down] = disk_bh_pro_spin_angles[indices_bh_spin_down] + spin_torque_iteration
    ...
    bh_new_spin_angles[bh_new_spin_angles < disk_bh_spin_minimum_resolution] = 0.0
